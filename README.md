...
- **傾きで演奏** — 2つのモードを切替可能：
  - **軸モード**：左右の傾き（gamma）で音程、前後の傾き（beta）で明るさ。
  - **中心モード**：中心で低音、どの方向でも中心から離れるほど高音に。
- **6 種類の音色** — サイン / ウォーム / スクエア / ノコギリ / スーパーソウ / FMベル。
- **振動でジャンプ音** — スマホを強く振ると「びよよーん」（ON/OFF 可）。
- **調整スライダー** — 感度・変化の速さ・音域・明るさ幅・音量。
- **音階スナップ** — メジャー / マイナー / ペンタトニック / ブルース / 半音階 / 全音音階。
- **設定を記憶** — 選択内容を localStorage に保存し次回復元。
- **共有 / QRコード** — 設定内から共有・リンクコピー・QR 表示。
- **スクロールしない1画面 UI** — 設定は右上の ⚙ から開くシート。
- **モバイル最適化** — iPhone のカメラ（ノッチ/Dynamic Island）と Android ナビゲーションバーを考慮（fullscreen 表示）。
